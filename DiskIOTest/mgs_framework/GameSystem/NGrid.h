...
{
public:
	GridInfo() : i_timer(0), i_unloadflag(false){}
    ...
    void ResetTimeTracker() { i_timer.Reset(); }
...
class ThreadModel = SingleThreaded<ACTIVE_OBJECT>
>
class MANGOS_DLL_DECL NGrid
{
    public:
        ...
        NGrid(uint32 id, int32 x, int32 y, time_t expiry, bool unload = true) : 
			i_gridId(id), i_cellstate(GRID_STATE_INVALID), i_x(x), i_y(y), i_GridObjectDataLoaded(false), child(NULL)
            {
                i_GridInfo = GridInfo(expiry, unload);
                i_GridIdleInfo =  GridInfo(20*expiry, false); 
            }
		NGrid(){}
		void Init(uint32 id, int32 x, int32 y, time_t expiry, bool unload = true)
		{
			i_gridId = id;
        ...
        void ResetTimeTracker() { i_GridInfo.ResetTimeTracker(); }
        void UpdateTimeTracker(time_t diff) { i_GridInfo.UpdateTimeTracker(diff); }

         const TimeTracker& getIdleTimeTracker() const { return i_GridIdleInfo.getTimeTracker(); }
         void ResetIdleTimeTracker(time_t interval) { i_GridIdleInfo.ResetTimeTracker(interval); }
         void ResetIdleTimeTracker() { i_GridIdleInfo.ResetTimeTracker(); }
         void UpdateIdleTimeTracker(time_t diff) { i_GridIdleInfo.UpdateTimeTracker(diff); }
        ...
        //返回Grid中原有载入物体的个数
        unsigned int SelfObjectsInGrid(void) const
        {
            unsigned int count=0;
            for(unsigned int x=0; x < N; ++x)
                for(unsigned int y=0; y < N; ++y)
                    count += i_cells[x][y].SelfObjectsInGrid();
            return count;
        }
